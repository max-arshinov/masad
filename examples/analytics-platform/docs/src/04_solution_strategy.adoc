...
- Ingestion: Apache Kafka as durable backbone; ClickHouse Kafka Engine source tables with Materialized Views to transform and load into MergeTree tables; Schema Registry (Avro/Protobuf), dedup via event_id(+version), and DLQ. S3-staged loads for backfills. See link:../../adrs/0003-choose-ingestion-to-clickhouse.md[ADR-0003].
